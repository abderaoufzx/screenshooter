var Page = function (
  ...
  // Explicitly set clipRect to match viewport to ensure viewport-only screenshot
  if (opts.cropRect) {
    page.clipRect = {
      top: opts.cropRect[0],
    ...
    // Default to viewport size for screenshot
    page.clipRect = {
      top: 0,
      left: 0,
      width: opts.width,
  ...
  page.onResourceReceived = function (response) {
    if (response.stage && response.stage === "end") {
      if (!metadata.status && response.status && response.contentType && response.contentType.indexOf('text/html') !== -1) {
        metadata.status = response.status;
        metadata.headers = response.headers.reduce(function(map, obj) {
  ...
  function renderAndExit() {
    ...
    // Capture only the viewport as defined by viewportSize and clipRect
    page.render(opts.file, { format: opts.format, quality: opts.quality });

    // Write metadata JSON
    var fs = require("fs");
    var metaFilename = opts.file.replace(/\.[^/.]+$/, ".json");
    fs.write(metaFilename, JSON.stringify(metadata, null, 2));
...
function main() {
  ...
  if (
    ...
    URL.length == 0 ||
    typeof output_file === "undefined" ||
    output_file.length == 0
  ) {
    console.log(
      "Usage: phantomjs [options] screenshooter.js url_capture=<URL> output_file=<output_file.png> [header=<custom header> http_username=<HTTP basic auth username> http_password=<HTTP basic auth password>]"
    );
    console.log(
      "Please specify an URL to capture and an output png filename !"
    );
